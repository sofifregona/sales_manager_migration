...
import { getAllAccounts, getAccountById } from "~/feature/account/account-api.server";
// Types
...
export async function accountLoader({ request }: LoaderFunctionArgs): Promise<AccountLoaderData> {
  return runWithRequest(request, async () => {
    ...
    try {
      ...
      const parsed = parseAppError(error, "(Error) No se pudo obtener la lista de cuentas.");
    ...
    if (idParam) {
      ...
      try {
        ...
        const parsed = parseAppError(error, "(Error) No se pudo cargar la cuenta seleccionada.");
