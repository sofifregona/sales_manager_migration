...
import { getAllEmployees, getEmployeeById } from "~/feature/employee/employeeApi.server";
import type { EmployeeDTO, EmployeeLoaderData } from "~/feature/employee/employee";
import type { Flash } from "~/types/flash";
import { jsonResponse } from "~/lib/http/jsonResponse";
import { parseAppError } from "~/utils/errors/parseAppError";
import { validateRequiredId } from "~/utils/validation/validationHelpers";

export async function employeeLoader({ request }: LoaderFunctionArgs): Promise<EmployeeLoaderData> {
  return runWithRequest(request, async () => {
    ...
    if (rawField && !["name", "active"].includes(rawField)) {
      parseAppError((flash.error = "(Error) Campo de ordenación inválido."), (flash.source = "client"));
    }
    ...
    if (rawDirection && !["ASC", "DESC"].includes(rawDirection.toUpperCase())) {
      parseAppError((flash.error = "(Error) Dirección de ordenación inválida."), (flash.source = "client"));
    }
    const sortDirection = (rawDirection as "ASC" | "DESC") ?? "ASC";

    let employees: EmployeeDTO[] | null = null;
    try {
      employees = await getAllEmployees(includeInactive, { sortField, sortDirection });
    } catch (error) {
      const parsed = parseAppError(error, "(Error) No se pudo obtener la lista de empleados.");
      throw jsonResponse(parsed.status ?? 500, { error: parsed.message, source: parsed.source ?? "server" });
    }
    ...
    if (idParam) {
      ...
      try {
        ...
        const parsed = parseAppError(error, "(Error) No se pudo cargar el empleado seleccionado.");
