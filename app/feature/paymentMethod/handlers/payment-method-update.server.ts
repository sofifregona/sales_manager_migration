...
import { updatePaymentMethod } from "~/feature/paymentMethod/payment-method-api.server";
import type { UpdatePaymentMethodPayload } from "~/feature/paymentMethod/payment-method";
...
export async function handlePaymentMethodUpdate({ url, formData }: Ctx) {
  const nameParam = formData.get("name");
  const nameErr = validateRequiredAndType(nameParam, "string", "Nombre");
  if (nameErr) return jsonResponse(422, nameErr);
  const name = (nameParam as string).trim();

  const idAccountParam = formData.get("idAccount");
  const idAccountReq = validateRequiredAndType(
    idAccountParam,
    "string",
    "Cuenta",
  );
  ...
  const payload: UpdatePaymentMethodPayload = { id, idAccount, name };
  try {
    ...
    if (parsed.status === 409) {
      ...
      if (code === "PAYMENT_METHOD_EXISTS_INACTIVE") {
        ...
        headers.append(
          "Set-Cookie",
          makeConflictCookie({ scope: "payment-method", name, idAccount }),
        );
        return redirect(
          `/settings/payment-method?${p.toString()}` as any,
          {
            headers,
          } as any,
        );
