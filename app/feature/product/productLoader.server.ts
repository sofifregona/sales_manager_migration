...
import { getListOfProducts, getProductById } from "./productApi.server";
...
export async function productLoader({
  ...
  return runWithRequest(request, async () => {
    ...
    const incremented = url.searchParams.get("incremented") === "1";
    ...
    const flash: Flash = { created, updated, deleted, incremented };
    ...
    if (idParam) {
      ...
      const idError = validateNumberId(id, "Proveedor");
      if (idError) {
      ...
      try {
        ...
        const parsed = parseAppError(
          error,
          "(Error) No se pudo cargar la proveedor seleccionada."
        );
