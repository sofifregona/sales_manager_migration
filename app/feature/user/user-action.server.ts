...
//   updateUser,
//   deactivateUser,
// } from "~/feature/user/user-api.server";
...
import { handleUserReactivateSwap } from "./handlers/user-reactivate-swap.server";
...
export async function userAction({ request }: ActionFunctionArgs) {
  return runWithRequest(request, async () => {
    ...
    switch (parsed.intent) {
      case "reactivate":
        return await handleUserReactivate({ formData });
      case "reactivate-swap":
        return await handleUserReactivateSwap({ formData });
      case "deactivate":
        return await handleUserDeactivate({ formData });
      case "create":
