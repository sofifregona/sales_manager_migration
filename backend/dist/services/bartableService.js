...
import { validateIsInteger, validateIsPositive, validateNumber, validateNumberID, } from "../utils/validations/validationHelpers.js";
// SERVICE FOR CREATE A BARTABLE
export const createBartable = async (data) => {
    const { number } = data;
...
export const getBartableById = async (id) => {
    validateNumberID(id);
    const existing = await bartableRepo.findOneBy({ id, active: true });
    if (!existing)
        throw new AppError("Mesa no encontrada", 404);
