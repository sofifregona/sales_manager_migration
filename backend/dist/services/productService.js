...
import { validateNumberID, validatePositiveInteger, validatePositiveNumber, validateRangeLength, } from "../utils/validations/validationHelpers.js";
import { IsNull, Like, Between, MoreThanOrEqual, LessThanOrEqual, Raw, In, } from "typeorm";
...
export const updateProduct = async (updatedData) => {
    const { id, name, code, price, idProvider, idCategory, idBrand, active } = updatedData;
    validateNumberID(id);
...
export const getListOfProducts = async (params) => {
    const { name, code, minPrice, maxPrice, idProvider, idCategory, idBrand, sortField, sortDirection, } = params;
        ...
        ...(code?.trim()
            ? (() => {
              ...
              LPAD(
                ...
                REGEXP CONCAT('^0*', :prefix)`, { w, prefix }),
                };
            })()
            : {}),
